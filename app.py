...
if user_input:
    normalized_input = user_input.strip().lower()  # ✅ Normalize input to lowercase
    st.session_state.messages.append({"role": "user", "content": user_input})
    st.chat_message("user").write(user_input)

    if normalized_input in ["help", "?"]:
        response = get_help_message()
    else:
        try:
            result = agent.invoke({"input": normalized_input})  # Use normalized version
...
st.markdown("<small>Type 'help' to see example formats.</small>", unsafe_allow_html=True)
